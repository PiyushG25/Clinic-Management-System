  ...
  try {
    ...
    try {
      const userdata = await user.findOne({
        _id: await payload.userId,
        user_role: await payload.role,
        status: true,
      });
      if (userdata)
        return res.send({
...
async function getUserData(req, res) {
  ...
  if (req.body.user_name != undefined && req.body.phone_no != undefined) {
  ...
  if (req.body.user_name === undefined && req.body.phone_no !== undefined) {
    const datas = await user.find({ phone_no: req.body.phone_no, status: true });
    if (datas) {
      return res.send({ success: datas });
    } else {
      return res.send({ error: "No such user found" });
    ...
    if (!usernameexist) {
      try {
        const newdata = await user.findOneAndUpdate(
          { _id: req.body.userId, status: true },
          {
            user_name: req.body.user_name,
            user_role: req.body.user_role,
            phone_no: req.body.phone_no,
